...
1. **Prep + chunk** (deterministic):
   ...
   `name, color, rarity, types, gih_wr, iwd, alsa, oh_wr, gd_wr, play_rate, <grade>_grade, notes{LoL,Numot,LR,LLU}`.
...
gih_wr = Games-in-Hand WR (primary) · iwd = improvement-when-drawn (pp) · alsa = avg last seen at
