# Ravnica Allegiance (RNA) — Draft Guide

> **Coverage note — 3 gameplay episodes, no dedicated theory episode.** All three sources are "Showdown" live-draft videos (Ethan vs. Ben). There is no preview/crash-course or retrospective episode sampled. Insights are incidental commentary extracted from real-time draft decision-making. Treat every tier and evaluation here as weakly-sourced practitioner read, not data. **17Lands GIH WR is the only reliable primary signal for this set.**

## ⚠ Recency rule (read first)

- Notes span **three "Showdown" episodes** dated **2019-01-26 → 2019-03-27** (roughly the first 9 weeks of RNA limited).
- All three episodes are the **same phase** — mid-format gameplay reps — with no preview content and no retrospective. The latest episode (2019-03-27, Showdown 3) gets the most weight as the settled read.
- Both hosts' takes evolved noticeably across the series: early Showdown 1 comments on guild rankings were treated as provisional; by Showdown 3 comments are more confident and cross-validate each other.
- **On any conflict, Showdown 3 (2019-03-27) is the authoritative read.** Earlier episodes are corroborating evidence.
- **17Lands GIH WR stays the primary signal.** This guide is Lords of Limited practitioner color commentary only.

## Source timeline

| Date | Episode title | Phase | Weight |
|------|--------------|-------|--------|
| 2019-01-26 | Ravnica Allegiance Showdown 1 | Mid-format gameplay (early reps, admitted "still figuring things out") | Low — real-time commentary, explicitly provisional |
| 2019-02-28 | Ravnica Allegiance Showdown 2 | Mid-format gameplay | Medium — more reps, more confident takes |
| 2019-03-27 | Ravnica Allegiance Showdown 3 | Mid-format gameplay (most settled read) | Highest — most authoritative across this set |

## TL;DR meta read

- **Orzhov (WB) is the best guild, Azorius (WU) is second.** Both hosts stated this explicitly in Showdown 2. Ben's confirmed ranking: Orzhov > Azorius > Rakdos > then lower. White is called the "best color" across all three episodes.
- **Azorius Skies is the premium archetype** — tempo-oriented flyers backed by hard removal (Summary Judgment, Law-Mage's Binding), life gain (Impassioned Orator, Senate Guild Mage), and card draw (Depose/Deploy, Senate Guild Mage). Senate Guild Mage specifically was praised as "ridiculous, ridiculous strong" and one of the best two-drops.
- **Orzhov is removal-dense and resilient** — Final Payment, Grotesque Demise, Night of the Last Breath, Consecrate/Consume, plus Angelic Exaltation as a finisher. The key tension: "we need about 15 creatures" to support Angelic Exaltation and Blade Juggler.
- **Guild gates are high picks, especially Gateway Plaza.** Both hosts emphasized guildgates far higher than normal. Gate Colossus is a real payoff; gateway package (Open the Gates, Gateway Sneak) is "sweet." Multicolored decks with gate fixing are common and powerful.
- **The format can stall into board grinds** — both hosts noted "board stall, the format" and Kaya's Wrath is a real punisher for over-extension. Clear the Mind as a main-deck card was mentioned (Ben: "I main-decked my first Clear the Mind today").
- **Five-drop slot is clogged.** Repeatedly referenced as the "five-drop tax": many good fives (Chill Bringer, Azorius Knight-Arbiter, Watchful Giant, Debtor's Transport) mean you don't need to prioritize any single one.
- **Gruul/Simic are the weaker guilds.** Gruul can win but "people have kind of decided it's the worst guild so it's been open lately" (Showdown 3). Simic is underdrafted but pulling off a clean deck is harder to signal.
- **Removal is scarce — hard exile removal is premium.** Summary Judgment and Grotesque Demise repeatedly prioritized early. Law-Mage's Binding and Slimebind are notable but creature-type-gated or sorcery-speed.

## Archetype / color-pair tiers

> Evidence is from gameplay-only commentary. Thin on Simic, Rakdos depth. Tiers reflect the hosts' stated preferences at the time of latest episode (2019-03-27).

| Pair | Colors | Tier | Game plan | Key commons/uncommons | Notes |
|------|--------|------|-----------|----------------------|-------|
| **Orzhov** | WB | **S** | Afterlife/token value, removal density, Angelic Exaltation beatdown; resilient grind | Final Payment, Grotesque Demise, Consecrate/Consume, Blade Juggler, Imperious Oligarch, Angelic Exaltation, Night of the Last Breath, Dead Revels | Ben's preferred guild; "the best guild." Afterlife tokens + Exaltation is the marquee finishing combo. Needs ~15 creatures. |
| **Azorius** | WU | **A** | Flyers + tempo removal + life gain, Senate Guild Mage as engine, Deputy of Detention as power spike | Summary Judgment, Depose/Deploy, Law-Mage's Binding, Senate Guild Mage, Impassioned Orator, Skitter Eel, Windstorm Drake | Ethan's preferred guild; "definitely a head of Rakdos." Senate Guild Mage called "ridiculous strong." High Alert as an optional build-around. |
| **Rakdos** | BR | **B** | Aggro/spectacle — low-curve 1s and 2s, Light Up the Stage, Cavalcade of Calamity build possible | Skewer the Critics, Fire Blade Artist, Footlight Fiend, Rakdos Trumpeter, Ill-Gotten Inheritance, Light Up the Stage, Goblin Gathering | Ben ran Cavalcade of Calamity (Showdown 3); noted "wide open" lane but "not a ton of powerful Rakdos cards opened." Ill-Gotten Inheritance was called important to the archetype's long-game. |
| **Gruul** | RG | **B/C** | Curve out beef, Rhythm of the Wild haste payoff, Troll-Bred Guardian as top end | Clan Crusher (Bull-Rack Clan Crusher?), Troll-Bred Guardian, Savage Smash, Rubble Belt Runner, Biogenic Upgrade | "People have decided it's the worst guild so it's been open." Rhythm of the Wild is the premier uncommon. Two-drops are critical and hard to find — main weakness. |
| **Simic** | GU | **C** | Adapt synergies, Frilled Mystic as tempo, Hydroid Krasis build-around | Sora-Form Hybrid (?), Skitter Eel, Frilled Mystic, Steeple Creeper | Rarely cleanly drafted in these episodes; Ben "saw no blue" in two of three drafts. Frilled Mystic noted as a card to respect if opponent is Simic. |
| **Gate deck** | 3–5c | **A (if open)** | Gate payoffs — Gate Colossus + Gateway Sneak + Open the Gates; multicolor fixing via gates | Gateway Plaza, all guild gates, Gate Colossus, Gateway Sneak, Open the Gates | Both hosts called gates "very high picks." Ben: "gate payoffs are sweet, multi-colored decks are sweet." Gate Colossus cited as first-pickable from pack 2 onward. |

## Format principles

Extracted from all three episodes. More-recently stated principles take precedence.

### Draft / signals
- **White is the best color; lean toward white guilds.** Ethan in Showdown 1: "leaning towards white guilds, leaning away from green guilds." Confirmed through Showdown 3.
- **Guild gates are first-pick-worthy.** Gateway Plaza especially — "this gate is very very strong in conjunction with Open the Gates." Ben: "I think guildgates are super high picks." Take them even in mid-pack if the gate payoffs are there.
- **Read whether Orzhov or Azorius is more open.** Both share white; getting cut by the other white guild is the main risk. Watch for Summary Judgment and Grotesque Demise/Consecrate/Consume signals.
- **Don't pick gates in pack 3 unless you have real payoffs.** Ben: "I take gates a little less highly [in pack 3] unless I've got really good gate payoffs or a high density of them."
- **The five-drop tax.** Don't first-pick five-drops — there are so many good ones that they flow late. Prioritize removal, two-drops, and bombs first.
- **Orzhov specifically wants ~15 creatures** to enable Angelic Exaltation and give Blade Juggler reliable card draw.

### Deckbuilding
- **17 lands is the baseline.** Both hosts consistently built 17-land decks. Azorius with heavy card draw (Senate Guild Mage + Sphinx's Insight + Depose/Deploy) can consider 18.
- **Lockets are playable as 1-ofs.** Ben and Ethan both ran one Orzhov or Azorius Locket for fixing and late-game card draw. Two lockets is sometimes correct in Azorius with expensive spells.
- **Mana bases: ~10/10 for two-color decks, heavier on the dominant color.** Azorius: Ethan ran 11/8 (blue-heavy for Skitter Eel, Mass Manipulation). Adjust for gold cards.
- **Life gain matters.** Impassioned Orator + Depose/Deploy tokens = significant life swing. Senate Guild Mage draining for two each turn closes games. "This gains you so much life throughout a game."
- **Clear the Mind is a real card in this format.** Long games happen; both hosts mentioned main-decking or seriously considering it. Against grind decks (other Azorius, Orzhov), it's a legitimate strategy.
- **Sideboard Exposed to Daylight.** Enchantment-based removal (Law-Mage's Binding, Slimebind) and enchantment payoffs (Ill-Gotten Inheritance, High Alert, Dovan's Acuity) make Exposed to Daylight a live sideboard card in nearly every matchup.

### Play patterns
- **Azorius vs. Orzhov is a tough matchup for Orzhov** — Azorius flyers can go over the top and Orzhov's ground-based Angelic Exaltation plan struggles. Ethan noted: "If he drafted flyers we're in big trouble, that's like Orzhov's toughest matchup."
- **Don't over-extend into Kaya's Wrath.** It's a real card in Orzhov sideboards/main decks. The format goes long enough that this comes up.
- **Mass Manipulation wins games in stalls.** Taking Gate Colossus or a key blocker is "stupid" levels of value and ends grinding games. If you're Azorius with significant blue mana, Mass Manipulation is maindeckable.
- **Spectacle enablement.** Rakdos specifically needs reliable turn-1 or turn-2 damage to fire off spectacle. Footlight Fiend and Rakdos Trumpeter are key enablers; Cult Guild Mage deals incidental damage.
- **Resolute Watchdog is a strong sideboard card.** In Azorius vs. Orzhov, blanking Imperious Oligarchs and surviving Wraths are both relevant. Ethan sided it in specifically for those reasons.

## Card notes (community consensus)

> Only cards explicitly discussed during the draft commentary are listed. Caption garble noted with (?).

### White
- **Summary Judgment** — Best white common. "Best white common, best common in most packs it's in." Ethan first-picked it repeatedly when available. Two-mana over three-mana is relevant for tempo.
- **Impassioned Orator** — Called "second-best white common." Life gain is significant in racing scenarios and with token strategies (Depose/Deploy). Strong two-drop.
- **Tithe Taker** — Strong two-drop with relevant tax ability. Ben debated Tithe Taker vs. Orzhov Enforcer: concluded Enforcer is slightly better defensively but Tithe Taker "hits a little bit harder."
- **Orzhov Enforcer** — Ben's conclusion: slightly better than Tithe Taker as a defensive wall vs. Gruul/Simic. Flying deathtouch afterlife creature that "literally says Gruul and Simic can't attack you on the ground."
- **Imperious Oligarch** — "Bread and butter two-drop of the Orzhov deck." Ben and Ethan both first-picked it early. All three imperious oligarchs wheeled back to Ben in Showdown 1 at very late picks.
- **Concordia Pegasus** — Cheap two-drop flyer; value goes up with Windstorm Drake and Depose/Deploy tokens.
- **Watchful Giant** — "Useful big butt"; upside with Angelic Exaltation but otherwise just a defensive five-drop. Acceptable filler.
- **Resolute Watchdog** — Sideboard in vs. Orzhov (blanks Oligarchs, protects against sacrifice/removal). Can be maindecked in High Alert builds.
- **10th District Veteran** — Fine filler; noted as "much better than Haazda Officer" in Orzhov where High Alert is possible. Creature for Exaltation.
- **Haazda Officer** — Called "low impact" and cut from Ethan's final Azorius deck. Not recommended as a maindeck card unless you have no other options.

### Blue
- **Senate Guild Mage** — "Ridiculous, ridiculous strong magic card." Ethan: "One of the best two-drops." The five-mana drain ability "closes out games so fast." Taking two is actively better — they provide consistency and life gain stacks. Snap first-pick.
- **Skitter Eel** — Higher than most people think. Ethan: "Ben is gonna be so mad about this card, he hates this card so much." The 3/5 body after adapting holds the ground while flyers win the air, and the five-mana ability is relevant. Good in Azorius.
- **Chill Bringer** — Good five-drop; tapping a blocker every attack is relevant for flying-heavy Azorius. Often preferable to Knight-Arbiter unless you have High Alert.
- **Azorius Knight-Arbiter** — "Fine five-drop." Good with High Alert (8 toughness vigilance), okay without. Subject to the five-drop tax — don't prioritize early.
- **Windstorm Drake** — "Really nice" for Azorius; turns all flying Depose/Deploy tokens into relevant two-power attackers and buffs Senate Courser to 2/4. Pick up when available.
- **Depose/Deploy** — Premium Azorius split card. "Gasoline." Taking two is great; they provide tempo (Depose) and board development (Deploy tokens) with life gain. "Basically the wheel" early in format.
- **Deputy of Detention** — "In the best color pair, like probably slightly worse than [Rakdos Fire-Wheeler] but I really like it." Blue's best rare/uncommon.
- **Law-Mage's Binding** — Strong unconditional removal for blue. Note: enchantment removal makes it weak in Game 2 if opponent has Exposed to Daylight effects.
- **Slimebind** — Ethan raised his opinion over time. "A card I'm much higher on than I was before." Best used on the most relevant creature on board; weak vs. large Orzhov creatures. Sideboard out vs. opponents with enchantment removal.
- **Essence Capture** — "Pretty undervalued" per Ethan in Showdown 3. Don't sleep on it.
- **Shimmer of Possibility** — "If you want, you can get a copy of and I generally don't want a copy." Ethan preferred it when actively digging for a specific payoff (e.g., High Alert, Dovan's Acuity), but not generically great.
- **Sphinx's Insight** — Ethan debated Sphinx's Insight vs. Sage's Revenant (?): concluded card draw is better than the 2/1 body when you have multiple sources (Senate Guild Mage). Good in acuity-style decks.
- **Code of Constraint** — Ethan took it over Shimmer in one draft while hoping for Dovan's Acuity. Said: "I don't really want Code of Constraint very much" — poor tempo orientation in a non-aggressive Azorius build. Wrestled Admonition often preferred.
- **Swirling Torrent** — "A card I don't think I'll play" in control Azorius; "sort of a Simic card." Useful from the sideboard to bounce back stolen creatures (e.g., Mass Manipulation counter-play).
- **Mass Manipulation** — Situational bomb; needs significant blue sources but "just gives you so much inevitability" in stalled board states. Game-ending in combination with Gate Colossus or large Orzhov threats.
- **Emergency Powers** — "Not a good card." The symmetry is the problem; Ethan explicitly passed it.
- **Senate Courier** — "Best blue common" — called that by both hosts. Fine 2/3 vigilance that attacks and blocks. Noted as a strong common despite not being flashy.

### Black
- **Grotesque Demise** — "Super strong removal spell." Exile clause is "really relevant against spectacle." Ethan called it the best black common. Priority pick in Orzhov.
- **Consecrate/Consume** — "Absurdly powerful" removal in Orzhov. Ben: "There are not a lot of cards I would take over Consecrate/Consume in the set." Took it twice in Showdown 2 and once in Showdown 3.
- **Final Payment** — Best black removal. Ben assumed opponents would first-pick it; noted as the reason to take Angelic Exaltation second in Showdown 2.
- **Night of the Last Breath** — Good removal; exiles two creatures. Ben took it over Senate Guild Mage (two-way Azorius guild gate) because "I don't think I need to stretch my mana that much."
- **Angelic Exaltation** — "Stone cold bomb" in Orzhov creature decks. Ben's key finisher. "The only way we're going to be in trouble is if Ethan drafted flyers." Best card to wheel if you can get it.
- **Ill-Gotten Inheritance** — "First copy would be very good, I would run multiple copies in this deck." Card advantage + life drain; "people are finally starting to pick that card higher." Important long-game card for Rakdos.
- **Blade Juggler** — "Card advantage, a creature for Angelic Exaltation." In Orzhov, needs ~15 creatures to get reliable value. Perfect pick in a creature-dense Orzhov build.
- **Dead Revels** — "Really value the first copy pretty highly in a black deck." Returns creatures from graveyard — important with Imperious Oligarch and Blade Juggler in Orzhov.
- **Syndicate Messenger** — Acceptable four-drop in Orzhov but "our four-drop slot gets super clogged in this format." Don't prioritize.
- **Carrying Imp** — Fine body; Ben took it in Showdown 1 as defensive filler while waiting to establish his guild.
- **Thirsting Shade** — Fringe playable in Orzhov; better if you have sacrifice outlets (e.g., Hero of Precinct One).
- **Undercity Scavenger** — Good in Rakdos against enchantment removal decks; grows large via sac. Ben ran it main deck in Showdown 3 specifically because it triggers off the cavalcade package and is good against enchantment-based removal.

### Red
- **Skewer the Critics** — Premium removal in both Rakdos and Gruul. "Ridiculous" tempo play when spectacle is active; still fine as a 3-mana sorcery. Better in Rakdos than Gruul.
- **Light Up the Stage** — Ben initially took it over Titanic Brawl. Ethan: "I'm a little lower on Light Up the Stage in limited these days." In Gruul, usually the last spell cast (expensive stuff means you often miss). In Rakdos it's better — fires off after dealing early damage. Both hosts ultimately ran it.
- **Scorchmark** — "That card is bad" (Ethan, Showdown 3, explicitly). Do not play.
- **Rakdos Fire-Wheeler** — "Best card" in Rakdos, good with spectacle, high floor/ceiling. Ben drafted it highly in Showdown 3.
- **Fire Blade Artist** — "Fine two-drop" but "gets blanked fairly early on" if the opponent has creatures. Good with Rakdos Trumpeter for spectacle enablement. "Inevitability" as a late-game source of chip damage.
- **Footlight Fiend** — Rakdos spectacle/cavalcade enabler. Deals 1 when it dies — relevant for spectacle and Cavalcade of Calamity. Ben ran it in Showdown 3.
- **Rakdos Trumpeter** — "Bread-and-butter type card of the Rakdos deck." Ben assumed he was "the only Rakdos drafter" when it wheeled last pick.
- **Goblin Gathering** — Useful in Cavalcade of Calamity builds — provides three 1/1 bodies. Ben specifically drafted around it in Showdown 3. Two or more copies makes Cavalcade payoffs much better.
- **Cult Guild Mage** — "Really just good at enabling spectacle, dealing some incidental damage." Discard ability is expensive. Good in Rakdos; worse in other archetypes.
- **Cavalcade of Calamity** — Ben built around it in Showdown 3 with good results. "Nine to ten cards that work with it" is the right density. Needs Footlight Fiend, Rakdos Trumpeter, Fire Blade Artist, and Goblin Gatherings as the package.
- **Act of Treason** — "Great pickup" in Gruul; "another card I'd like to try and find its way into this deck." Ben Showdown 3: considered it as a finisher-stealer.
- **Burn Bright** — Sideboard-viable in Rakdos; more relevant in Go-Wide builds vs. decks with high-toughness blockers. Uncertain as main deck.

### Green
- **Troll-Bred Guardian** — One of the best green commons. Ben took it first pick in Showdown 3 over the rare (Unbreakable Formation). "A really strong card, one of the better Riot commons in the set." Double Troll-Bred Guardian is an ideal Gruul endgame.
- **Rhythm of the Wild** — "Very good card." Ethan initially said "I'm a lot lower on Rhythm than I used to be but it's still a very good card." Ben won multiple games with it on curve. Key Gruul uncommon — makes everything haste or countered.
- **Biogenic Upgrade** — Better overrun finisher than Andrej's Four-Runners (?) in Gruul at six mana. Ben took it over the eight-drop. Can be a blowout with Troll-Bred Guardian.
- **Andrej's Four-Runners (?)** — "I think Endres Forerunners is a trap" (Ethan, Showdown 2). Better in theory than in practice — too expensive. Ethan took Biogenic Upgrade over it.
- **Savage Smash** — "Such a beating." Splash-worthy. Ben called the Savage Smash vs. Skewer the Critics pick "the most interesting pick by far" in Showdown 3 and took Savage Smash twice. Timur splash of Savage Smash is viable.
- **Rubble Belt Runner** — "Nice" three-drop. Ben came around: "I think it's good in Gruul, you're happy to play that card as your three-drop in Gruul." Trample + haste from Rhythm makes it a clock.
- **Soraform Hybrid (?)** (Sora-Form Hybrid) — Best Simic common. Ethan noted it as "rock solid, best common in most packs it exists in." Both drafters valued it very highly when in green.
- **Steeple Creeper** — Fine three-drop in Gruul; better in Simic where the pump helps. Ben maindecked it when short on playables.
- **Spike Wheel Acrobat (?)** (Spikewheeel Acrobat / Spike-Will Acrobat) — 2/2 with relevant triggered ability; took it over Stony Strength in one pick. Okay to include in gruel but not premium.
- **Titanic Brawl** — Serviceable fight spell, worse than Savage Smash. "Goes a little late." Acceptable to run 1-2 copies; "kind of goes late" means you can wheel them.
- **Stony Strength** — "I like this card a lot. Nice with the Clan Crusher." One-mana trick; wins combats and enables spectacle on turn 1. Better than Gift of Strength in Gruul.
- **Gift of Strength** — Acceptable trick, but Stony Strength and Storm Strike preferred in Gruul.
- **Burning Tree Vandal** — "I like Burning Tree Vandal quite a bit" as a rummage engine to smooth out land-heavy draws.

### Multicolor
- **Hero of Precinct One** — Good in gold-heavy decks (Orzhov 6+ gold cards). Ben: "That makes me wish I had taken Imperious Oligarch" when he opened it. Better the more gold cards you have. Getting a body off each gold spell with Angelic Exaltation is "super good."
- **Dovan's Acuity (?)** (Dovin's Acuity) — "I don't get to play those and I feel like I'm pretty well set up for that right now." Both hosts described wanting it every time it wasn't in their deck. "I would have happily played Dovin's Acuity" — game-warping in Azorius.
- **High Alert** — "If we can get a High Alert I will be thrilled." Transforms Knight-Arbiters, Skitter Eels, and Resolute Watchdogs into threats. Very build-around but powerful. Ben noted: "We didn't get the High Alert I passed."
- **Kaya's Wrath** — "Good one" — both hosts respected it as a sweeper. Ben: "Let's snap that up, feels good to read signals correctly for Orzhov." Sideboard in if opponent is going wide; Orzhov can have it in main deck.
- **Senate Guild Mage** — (see Blue section above; listed again — it's Azorius gold) Ethan described taking two of them as "so much consistency."
- **Angelic Exaltation** — (see Black section above; it's Orzhov gold) "Stone cold bomb in our deck."
- **Syndicate Guild Mage / Cult Guild Mage** — Each has a specific role. Syndicate Guild Mage (Orzhov) is "good" but Ethan: "I think it's the worst guild mage." The five-mana drain ability is what actually closes games though.
- **Consecrate/Consume** — Reiterating: top-tier Orzhov removal. "There are not a lot of cards I would take over Consecrate/Consume in the set."

### Colorless
- **Gate Colossus** — Snap pick in any gate-heavy deck. "There's a reason to start taking gates a little bit higher." Came around pick 8-9 in Showdown 1, signaling the gate archetype was underdrafted. Ben ran it in Orzhov splash-gate.

### Lands
- **Gateway Plaza** — "This gate is very very strong in conjunction with Open the Gates." Ethan called it one of the highest picks early; enables any color splash and counts for Gate Colossus. First pick from some packs.
- **All guild gates** — Both hosts took guild gates proactively, especially in packs 1 and 2. "Gates are high picks." Orzhov Guildgate and Azorius Guildgate most valued for the two white guilds. In pack 3 take them less highly unless you have payoffs.

### Rares & bombs
- **Prime Speaker Vannifar** — Ben: "I don't think Prime Speaker is that good in draft. You're not going to draft a birthing-pod deck, that's hard to even do in cube." He first-picked it only because it was worth money and nothing else was clearly better.
- **Wilderness Reclamation** — Ethan considered splashing it in Simic but decided against it. "There's no world where we're going to want to play Wilderness Reclamation" in a Gruul/Timur pile.
- **Smothering Tithe** — "Could be really really good fixing" in a sideboard that goes wide with gates. Ben took it as a sideboard speculation only, not expecting to main deck it.
- **Unbreakable Formation** — "Rock solid combat trick." Ben: "always playing this in any white deck with 13-plus creatures." Good instant-speed combat trick or protect-the-team play.
- **Frilled Mystic** — "Definitely a card" if opponent is Simic. Respected but rarely drafted cleanly in these episodes.
- **Deputy of Detention** — "In the best color pair." Takes out any permanent type; Ethan first-picked it over Rakdos Fire-Wheeler in Showdown 3 because of the color commitment.

## Signals & seat reads

- **White being open is the primary read.** If Summary Judgment or Grotesque Demise wheels, you're clear. If Imperious Oligarch and Depose/Deploy both table, white is very open.
- **If you see late blue cards and no Azorius drafters, lock in Azorius.** Ethan multiple times noted "continuing to see no blue" as a strong Azorius signal.
- **Gate Colossus or Gateway Sneak wheeling = Gate deck is underdrafted.** These are premium payoffs; if they table you're in a good seat for the gate strategy.
- **Rakdos cards going very late = Rakdos is open.** Ben wheeled Rakdos Trumpeter last pick in Showdown 3. That is a "bread-and-butter" Rakdos card that shouldn't table in a contested pod.
- **Consecrate/Consume going 12th pick = opponent is NOT in Orzhov.** Ethan noted Ben passing it to him late in Showdown 3 as confirmation Ben wasn't in black-white.
- **Pivot signals:** If white starts getting cut, green going late is an opening for Gruul. Troll-Bred Guardian wheeling or Titanic Brawl tabling = Gruul is open. Be willing to abandon early picks for a lane that's genuinely open if you're not more than 4 picks deep.
- **Don't share white guilds at the same table if you can help it.** Ben and Ethan's main tension across all Showdown episodes was both leaning white. The person who reads the correct white guild gets a much better deck.

## Supersessions (early-was-wrong, now corrected — NOT live disagreements)

- **Senate Courier undervalued early → confirmed as "best blue common" (2019-01-26 → 2019-02-28+).** In Showdown 1, Ethan treated it as filler ("senate courier is not exciting at all but it's totally fine"). By Showdown 2-3, both hosts called it "best blue common." Verdict: Senate Courier is a real card — do not pass it early.
- **Slimebind dismissed early → meaningfully higher evaluation later.** Ethan in Showdown 1: "a card I'm much higher on than I was before," indicating his prior take was lower. The card has real application in Azorius as a tempo tool against large ground creatures.
- **Skitter Eel initially disliked by Ben → corrected to "pretty good."** Ben expressed distaste for the card in Showdown 1; by later games it was clearly pulling its weight in Azorius. Ethan: "Ben is gonna be so mad about this card, he hates this card so much." Performance proved it out.
- **Rhythm of the Wild over-valued early → downgrade acknowledged.** Ethan in Showdown 2: "I am a lot lower on Rhythm than I used to be but it's still a very good card." Still good, but expectations were revised downward from its early-format reputation.

## Live disagreements (genuine, unresolved)

- **Light Up the Stage: Ethan is notably lower than Ben.** Ethan: "I'm a little lower on Light Up the Stage in limited these days" across multiple drafts. Ben treats it as a keep-the-hand-flowing tool and seems more willing to run it. Not fully resolved.
- **Andrej's Four-Runners (?) vs. six-drops.** Ethan called it "a trap" in Showdown 2; Ben expressed less hostility to it in other contexts. Evidence thin — Ethan's take is stronger.
- **Two Rhythm of the Wilds in a deck.** Ethan strongly dislikes it ("you can have so many slots for puzzle pieces"). Ben hasn't explicitly addressed. One is consensus great; two is genuinely contested.

## Card-name caveats from caption garble

Auto-captions consistently mangled these names:

- **"Andrej's Four-Runners" / "Endres 4runners"** — Almost certainly *End-Raze Forerunners* (the Gruul overrun eight-drop)
- **"Soraform Hybrid" / "Sora-Form Hybrid"** — Almost certainly *Sauroform Hybrid* (Simic common, 3/3 creature)
- **"Bull Rack Clan Crusher"** — Almost certainly *Zhur-Taa Goblin* (?) or *Bull Clan Crusher* (?) — context suggests a Gruul 5-drop that removes blockers; possibly *Rampaging Monument* or *Kraul Harpooner*... more likely *Bull-Rack Clan Crusher* is *Bolrac-Clan Crusher* (Gruul uncommon 5/5 that removes +1/+1 counters to deal damage)
- **"Dovan's/Dovin's Acuity"** — Almost certainly *Dovin's Acuity* (Azorius enchantment life gain / draw)
- **"Dovan Insecurity" / "Dope Insecurity"** — Almost certainly *Dovin's Acuity* (same card, different garble)
- **"Hacker Bat"** — Almost certainly *Hackrobat* (Rakdos hybrid creature)
- **"Grotestemize"** — *Grotesque Demise*
- **"Snacky eel" / "Skitter eel"** — *Skitter Eel* (Simic adapt creature) — "Skitter eel" confirmed as the correct name in context
- **"Gravel Hyde Goblin"** — Likely *Gravelhi Goblin* (?) or *Gravel Hide Goblin* — Gruul common
- **"Tithe Taker"** — Correctly transcribed; appears as "taker of tithes" occasionally, clearly the same card
- **"Spike Wheel Acrobat" / "Spikewheel Acrobat"** — *Spire Patrol* (?) or more likely *Axebane Beast* (?) — context unclear; Ethan picked it as a flexible three-drop in Gruul; possibly *Spikefield Hazard* or *Aeromunculus* given the five-power reference
- **"Airmonkeylis" / "Air Monk Ulis"** — *Aeromunculus* (Gruul-adjacent flying creature with pump ability)
- **"Rubble Belt Runner"** — Likely *Rubblebelt Runner* (Gruul common haste creature)
- **"Rubble Slinger"** — Likely *Rubble Slinger* (?) or *Rubblebelt Recluse* — context unclear
- **"Feral Maka / Ferral NACA"** — *Feral Maaka* (red 2/3 cat, Gruul common)
- **"Regenesis" / "Repudiate Replicate"** — Likely *Repudiate/Replicate* (Simic split card)
- **"Birdie Circle"** — Likely *Pteramander* (?) or *Huatli's Raptor* — Ethan said "birdie circle is not good, I was so high on this card early and it's so powerful I just don't feel like it slots in a lot, it's just so very slow" — likely *Wilderness Reclamation* misread or *Precognitive Perception* mismatch; unclear
- **"Savant/Sage's Roosevant"** — *Sage's Row Savant* (Azorius two-drop)
- **"Smelt Morning.this" / "Smelt or Dignas"** — Likely *Smelt/Mizzium Blast* (?) or *Smelting Vortex* (?) — context suggests a sideboard artifact removal card

## Source episodes

- 2019-01-26 — Ravnica Allegiance Showdown 1 (haKoOwB8g9s)
- 2019-02-28 — Ravnica Allegiance Showdown 2 (3jgQy3fMAJ8)
- 2019-03-27 — Ravnica Allegiance Showdown 3 (eFBDDf9Akec)
