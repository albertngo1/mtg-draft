...
def main():
    ...
    while i < len(args):
        a = args[i]
        if a in ("pull", "rank", "resolve", "warm", "pool", "watch", "capture", "draft", "cards", "sealed"):
            cmd = a
    ...
    if cmd == "draft":
        ...
        pool, event = pull_sealed(cfg)
        apply_event(cfg, event)          # set/fmt from the sealed event's InternalEventName
        uniq = list(dict.fromkeys(str(i) for i in pool))   # de-dupe, preserve first-seen order
        proxy = "" if cfg["colors_explicit"] else "  (pass --colors to mark a build on-color)"
        print(f"\n  >> SEALED POOL — {event or 'sealed event'}: "
              f"{len(pool)} cards, {len(uniq)} unique{proxy}")
        print_table(uniq, cfg, show_text=not cfg["brief"])   # ranked stats + oracle text (build view)
        print_pool(pool, cfg, noun="cards")                  # category counts + curve (copies kept)
    elif cmd == "rank":
        if not ids:
